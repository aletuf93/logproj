...
def defineFromToTable(D_flows, colFrom, colTo, colQty):
    '''
    generate a from-to table from a flows table

    Parameters
    ----------
    D_flows : TYPE pandas dataframe
        DESCRIPTION. pandas dataframe containing the flows
    colFrom : TYPE string
        DESCRIPTION. column name containing the origin point of the flow
    colTo : TYPE string
        DESCRIPTION. column name containing the destination point of the flow
    colQty : TYPE string
        DESCRIPTION. column name containing the entity of each flow

    Returns
    -------
    output_dataframe : TYPE dict
        DESCRIPTION. dictionary of pandas dataframe with the from-to (flow count and quantity)
    output_figures : TYPE dict
        DESCRIPTION. dictionary of figures with the heatmaps (flow count and quantity)
