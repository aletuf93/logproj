...
def importGraph(caseStudy,nodecode):
    #restituisce un oggetto grafo G
    # e un dizionario con l'id_location associato al nodo del grafo
...
def analyseWhTraffic(D_mov_input, D_res, G, numPicks=-1, edgePredecessors=True, D_layout=[]):
    ...
    #D_mov è una tabella di movimenti
    #D_res è una tabella di corrispondenza fra idlocation e nodi del grafo
    #numPicks è il numero di picks da simulare
    #edgePredecessors è una variabile booleana per rappresentare il grafico
    #D_layout
    
    #la funzione ritorna un dataframe D_stat_arcs con i viaggi per arco
    # un dataframe D_stat_picks con le distanze per missione  di picking
...
def calculateExchangeSaving(D_mov_input, D_res, G, useSameLevel=False):
    ...
    D_mov = D_mov_input
    D_mov=D_mov_input.rename(columns={'IDLOCATION':'idlocation'})
...
def defineWHgraph(caseStudy, nodecode, idwh, whsubarea, vehicleCategory, draw=False, arcLabel=False, nodeLabel=False, trafficGraph=False):
    ...
    if len(D_layout)==0:
        ...
        #idlocation, rack, bay
        D_layout=qq.importGraph(caseStudy, nodecode, idwh, whsubarea, vehicleCategory,attributes=['IDLOCATION', 'RACK', 'BAY'])
        D_layout.columns=D_layout.columns.str.lower()
