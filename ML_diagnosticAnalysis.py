...
def getDescriptionKeywords(D_SKU,minFrequency,dirResults):
    ...
    dictionary=dictionary[dictionary.frequency>minFrequency]
    ...
    D_SKU['keyWords']='NullPredictor'
    for i in range(0,len(D_SKU)):
        descWords=D_SKU['cleanDescription'][i]
        if(not(descWords is None)):
            descWords=set(descWords.split(' '))
            keyWords=descWords.intersection(set(dictionary.index.values))
            salva=';'.join(str(s) for s in keyWords)
            D_SKU.at[i,'keyWords']=salva
    return D_SKU, dictionary, words_except_stop_dist